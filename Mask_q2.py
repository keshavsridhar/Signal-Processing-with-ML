...
if __name__ == "__main__":
    ...
    # plt.show()
    # M = np.divide(S.real, X.real)
    M_hat = np.divide(np.power(S, 2), (np.power(S, 2) + np.power(N, 2)))
    new_S = np.multiply(M_hat, X)
    # print(new_S)
    ...
    Y = (Y / np.max(Y)) * 32768
    Y = Y.astype(np.int16)
    scipy.io.wavfile.write("Piano_out_Mhat.wav", rate=piano[0], data=Y)
